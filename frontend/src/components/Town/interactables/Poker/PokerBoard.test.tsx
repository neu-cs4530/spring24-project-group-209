...
import { GameArea, GameStatus, PokerGameState } from '../../../../types/CoveyTownSocket';
...
describe('ConnectFourBoard', () => {
  ...
  async function checkBoard({
    ...
    for (let i = 0; i < cells.length; i++) {
      const cell = gameAreaController.board[Math.floor(i / 4)][i % 8];
      expect(cells[i]).toHaveAttribute(
        'aria-label',
        `Cell ${Math.floor(i / 4)},${i % 8} (${cell || 'Empty'})`,
      );
    }
