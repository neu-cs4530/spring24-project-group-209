...
class MockPokerAreaController extends PokerAreaController {
  ...
  mockPlayers: Map<SeatNumber, PlayerController | undefined> = new Map<
    SeatNumber,
    PlayerController | undefined
  >();

  mockCurrentGame: GameArea<PokerGameState> | undefined = undefined;
  ...
  mockClear() {
    this.mockBoard = new Array(9); // pulled from PokerAreaController class
    this.joinGame.mockClear();
    this.startGame.mockClear();
    this.raise.mockClear();
    this.call.mockClear();
    this.fold.mockClear();
  }
  ...
  isEmpty(): boolean {
    for (let i = 0; i < 8; i++) {
      if (this.mockPlayers.get(i as SeatNumber)) return false;
    }
    return true;
  }
...
describe('PokerArea', () => {
  ...
  beforeEach(() => {
    ...
    gameAreaController.startGame.mockReset();
    gameAreaController.call.mockReset();
    gameAreaController.raise.mockReset();
    gameAreaController.fold.mockReset();
    ...
    gameAreaController.call.mockImplementation(
      () =>
        new Promise<void>((resolve, reject) => {
          startGameResolve = resolve;
          startGameReject = reject;
    ...
    gameAreaController.raise.mockImplementation(
      () =>
        new Promise<void>((resolve, reject) => {
          startGameResolve = resolve;
          startGameReject = reject;
    ...
    gameAreaController.fold.mockImplementation(
      () =>
        new Promise<void>((resolve, reject) => {
          startGameResolve = resolve;
          startGameReject = reject;
  ...
  describe('[T3.1] Game Update Listeners', () => {
    ...
    it('Does not register a listener on every render', () => {
      ...
      const renderData = renderPokerArea();
      expect(addListenerSpy).toBeCalledTimes(2);
      addListenerSpy.mockClear();
    ...
    it('Creates new listeners if the gameAreaController changes', () => {
      ...
      gameAreaController = new MockPokerAreaController();
  ...
  describe('[T3.2] Join game button', () => {
    it('Is not shown if the game status is IN_PROGRESS', () => {
      ...
      gameAreaController.mockPlayers.set(
        0,
        new PlayerController('player zero', 'player zero', randomLocation()),
      );
      gameAreaController.mockPlayers.set(
        1,
    ...
    it('Is shown if the game status is WAITING_FOR_PLAYERS', () => {
      ...
      gameAreaController.mockPlayers.set(0, undefined);
      gameAreaController.mockPlayers.set(
        1,
        new PlayerController('player one', 'player one', randomLocation()),
      );
    ...
    describe('When clicked', () => {
      ...
      it('Adds the display of the button when a game becomes possible to join', () => {
        ...
        act(() => {
          gameAreaController.mockStatus = 'WAITING_FOR_PLAYERS';
          gameAreaController.mockPlayers.set(1, undefined);
  ...
  describe('[T3.3] Start game button', () => {
    ...
    it('Is not shown if the game status is WAITING_FOR_PLAYERS and there is only one player in the game', () => {
      gameAreaController.mockStatus = 'WAITING_FOR_PLAYERS';
      gameAreaController.mockPlayers.set(0, ourPlayer);
      gameAreaController.mockIsPlayer = true;
      renderPokerArea();
  ...
  describe('[T3.4] Players in game text', () => {
    it('Displays the username of each seat if there is one', () => {
      for (let i = 0; i < 8; i++) {
        gameAreaController.mockPlayers.set(
          i as SeatNumber,
          new PlayerController(nanoid(), nanoid(), randomLocation()),
        );
        gameAreaController.mockStatus = 'WAITING_FOR_PLAYERS';
        gameAreaController.mockIsPlayer = false;
        renderPokerArea();
        ...
        expect(
          within(listOfPlayers).getByText(
            `${i}: ${gameAreaController.mockPlayers.get(i as SeatNumber)?.userName}`,
          ),
        ).toBeInTheDocument();
      }
    });
    it('Displays no player yet for each empty seat', () => {
      for (let i = 0; i < 8; i++) {
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(i as SeatNumber, undefined);
        renderPokerArea();
        const listOfPlayers = screen.getByLabelText('list of players in the game');
        expect(within(listOfPlayers).getByText(`${i}: (No player yet!)`)).toBeInTheDocument();
      }
    });
    describe('Updates each player when the game is updated', () => {
      it('updates player 0 correctly', () => {
        const seatNumber: SeatNumber = 0;
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(seatNumber, undefined);
        renderPokerArea();
        const listOfPlayers = screen.getByLabelText('list of players in the game');
        ...
        expect(
          within(listOfPlayers).getByText(
            `${seatNumber}: ${gameAreaController.mockPlayers.get(seatNumber)?.userName}`,
          ),
        ).toBeInTheDocument();
      });
      it('updates player 1 correctly', () => {
        const seatNumber: SeatNumber = 1;
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(seatNumber, undefined);
        renderPokerArea();
      ...
      it('updates player 2 correctly', () => {
        const seatNumber: SeatNumber = 2;
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(seatNumber, undefined);
        renderPokerArea();
      ...
      it('updates player 3 correctly', () => {
        const seatNumber: SeatNumber = 3;
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(seatNumber, undefined);
        renderPokerArea();
      ...
      it('updates player 4 correctly', () => {
        const seatNumber: SeatNumber = 4;
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(seatNumber, undefined);
        renderPokerArea();
      ...
      it('updates player 5 correctly', () => {
        const seatNumber: SeatNumber = 5;
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(seatNumber, undefined);
        renderPokerArea();
      ...
      it('updates player 6 correctly', () => {
        const seatNumber: SeatNumber = 6;
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(seatNumber, undefined);
        renderPokerArea();
      ...
      it('updates player 7 correctly', () => {
        const seatNumber: SeatNumber = 7;
        gameAreaController.mockStatus = 'IN_PROGRESS';
        gameAreaController.mockIsPlayer = false;
        gameAreaController.mockPlayers.set(seatNumber, undefined);
        renderPokerArea();
  ...
  describe('[T3.5] Game status text', () => {
    ...
    describe('When a game is in progress', () => {
      ...
      it('Displays a message "Game in progress" and indicates whose turn it is when it is our turn', () => {
        renderPokerArea();
        expect(
          screen.getByText('Game in progress, currently your turn', { exact: false }),
        ).toBeInTheDocument();
      });
      it('Displays a message "Game in progress, {numMoves} moves in" and indicates whose turn it is when it is not our turn', () => {
        ...
        expect(
          screen.getByText(
            `Game in progress, 1 moves in, currently ${
              gameAreaController.mockPlayers.get(1)?.userName
            }'s turn`,
            { exact: false },
          ),
        ).toBeInTheDocument();
      ...
      it('Updates the turn when the game is updated', () => {
        ...
        expect(
          screen.getByText(
            `, currently ${gameAreaController.mockPlayers.get(1)?.userName}'s turn`,
            {
              exact: false,
            },
          ),
    ...
    describe('When the game ends', () => {
      ...
      it('Displays a toast with the loser', () => {
        ...
        renderPokerArea();
        gameAreaController.mockWinner = gameAreaController.mockPlayers.get(1);
        act(() => {
          gameAreaController.emit('gameEnd');
        });
        expect(mockToast).toBeCalledWith(
