...
import * as PokerGameModule from './PokerGame';
...
class TestingGame extends Game<PokerGameState, PokerMove> {
  public constructor(priorGame?: PokerGame) {
    super({
      ...
      foldedPlayers: new Array<boolean>(),
      pot: 0,
    });
  }
...
describe('PokerGameArea', () => {
  ...
  const gameConstructorSpy = jest.spyOn(PokerGameModule, 'default');
  ...
  describe('[T3.3] GameMove command', () => {
    ...
    describe('when there is a game in progress', () => {
      ...
      it('should call applyMove on the game and call _emitAreaChanged', () => {
        const move: PokerMove = { moveType: 'CALL' };
      ...
      describe('when the game ends', () => {
        test.each<SeatNumber>([0, 1, 2, 3, 4, 5, 6, 7])(
          ...
          (winner: SeatNumber) => {
            const finalMove: PokerMove = { moveType: 'CALL' };
